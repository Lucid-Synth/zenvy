...
function InputField({ id, label, type = "text", placeholder, value, onChange, error, icon, autoComplete }: InputFieldProps) {
  const [focused, setFocused] = useState(false);

  return (
    <div className="flex flex-col gap-1.5">
      <Label.Root htmlFor={id} className="text-xs font-medium text-zinc-400 tracking-wide">
        {label}
      </Label.Root>
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    ...
    >
      ...
      <p className={`text-xs ${textColors[strength]}`}>{labels[strength]} password</p>
    </motion.div>
  );
}
    ...
    <div className="relative min-h-screen bg-[#080808] text-white antialiased overflow-x-hidden selection:bg-violet-500/30 selection:text-violet-200 flex flex-col items-center justify-center px-6 py-16">
      ...
      >
        <a href="/" className="inline-flex items-center gap-2.5">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-linear-to-br from-[#a78bfa] to-[#6366f1] text-white text-sm font-black shadow-lg shadow-violet-500/30">Z</span>
          <span className="font-semibold tracking-tight text-white text-xl">Zenvy</span>
        </a>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {submitted ? (
          ...
          >
            ...
            >
              <motion.svg className="h-9 w-9 text-violet-400" viewBox="0 0 36 36" fill="none">
                <motion.path
                  d="M7 18l7 7 15-15"
                  stroke="currentColor"
                  strokeWidth="2.5"
            ...
            <div>
              <h2 className="text-2xl font-black tracking-tighter text-white mb-2">You're in. </h2>
              <p className="text-zinc-500 text-sm leading-relaxed">
                Welcome to Zenvy, <span className="text-zinc-300 font-medium">{form.name.split(" ")[0]}</span>! We've sent a confirmation to{" "}
                <span className="text-zinc-300 font-medium">{form.email}</span>. Check your inbox to get started.
              </p>
            </div>
            <a
              href="#"
          ...
          >
            {/* Header */}
            <div className="mb-7">
              <h1 className="text-2xl font-black tracking-tighter text-white mb-1.5">Create your account</h1>
              <p className="text-sm text-zinc-500">
                Already have one?{" "}
                <a href="/login" className="text-violet-400 hover:text-violet-300 transition-colors font-medium">Sign in</a>
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-xs text-zinc-600 shrink-0">or continue with email</span>
              <Separator.Root className="flex-1 bg-white/[0.07] h-px" />
            </div>
              ...
              <InputField
                ...
                icon={
                  <svg className="h-4 w-4" viewBox="0 0 16 16" fill="none">
                    ...
                    <path d="M1.5 6l5.97 3.5a1 1 0 0 0 1.06 0L14.5 6" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
                  </svg>
                }
              />
