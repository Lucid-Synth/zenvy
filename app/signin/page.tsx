...
function InputField({ id, label, type = "text", placeholder, value, onChange, error, icon, autoComplete, suffix }: InputFieldProps) {
...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    // Simulate a wrong-password error for demo
  ...
  return (
    <div className="relative min-h-screen bg-[#080808] text-white antialiased overflow-x-hidden selection:bg-violet-500/30 selection:text-violet-200 flex flex-col items-center justify-center px-6 py-16">
      ...
      >
        {/* Header */}
        <div className="mb-7">
          <h1 className="text-2xl font-black tracking-tighter text-white mb-1.5">Welcome back</h1>
          <p className="text-sm text-zinc-500">
            Don't have an account?{" "}
            <a href="/signup" className="text-violet-400 hover:text-violet-300 transition-colors font-medium">Sign up free</a>
          </p>
        </div>
          ...
          {globalError && (
            ...
            >
              <div className="flex items-center gap-2.5 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3">
                <svg className="h-4 w-4 shrink-0 text-red-400" viewBox="0 0 16 16" fill="none">
